...
from .errors import BlendRelayError, uid
...
class Service(Bridge, AssetService):
    ...
    def dispatch(self, method, params):
        if not isinstance(params, dict):
            raise BlendRelayError("INVALID_INPUT", "Parameters must be an object")
        ...
        with self.lock:
            ...
            if function is None:
                raise BlendRelayError("UNKNOWN_METHOD", method)
            try:
                ...
                raise BlendRelayError(error_code, str(error))
    ...
    def rpc_capabilities(self):
        return {
            "product": "BlendRelay MCP", "version": VERSION, "protocol_version": 1,
    ...
    def rpc_clear_data(self, include_assets=False, workspace_id=None):
        if workspace_id is not None:
            if not isinstance(workspace_id, str) or not re.fullmatch(r'[0-9a-f]{32}', workspace_id):
                raise BlendRelayError('INVALID_WORKSPACE', 'Invalid scene workspace ID')
    ...
    def rpc_source_register(self, path):
        ...
        if not any(source.is_relative_to(root) for root in self.source_roots):
            raise BlendRelayError("SOURCE_NOT_ALLOWED", "Start the runtime with --allow-source-root for this file location")
        supported = {".png", ".jpg", ".jpeg", ".webp", ".bmp", ".tif", ".tiff"}
        if source.suffix.lower() not in supported or not source.is_file():
            raise BlendRelayError("INVALID_SOURCE", "Use a supported raster reference image")
        if source.stat().st_size > 50 * 1024 * 1024:
            raise BlendRelayError("RESOURCE_LIMIT", "Reference image exceeds 50 MiB")
    ...
    def rpc_artifact_read(self, path, offset=0, limit=524288):
        ...
        if not any(artifact.is_relative_to(self.storage.root / name) for name in allowed) or not artifact.is_file():
            raise BlendRelayError("ARTIFACT_NOT_ALLOWED", "Only registered and generated visual artifacts can be read")
        if artifact.stat().st_size > 20 * 1024 * 1024:
            raise BlendRelayError("RESOURCE_LIMIT", "Artifact is too large")
        if not isinstance(offset, int) or not isinstance(limit, int) or offset < 0 or not 1 <= limit <= 524288:
            raise BlendRelayError("INVALID_RANGE", "Invalid artifact byte range")
        mime = {".png": "image/png", ".jpg": "image/jpeg", ".jpeg": "image/jpeg",
                ".webp": "image/webp", ".bmp": "image/bmp", ".tif": "image/tiff",
                ".tiff": "image/tiff"}.get(artifact.suffix.lower())
        if not mime:
            raise BlendRelayError("UNSUPPORTED_ARTIFACT", "Unsupported visual artifact")

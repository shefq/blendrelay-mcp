...
import time
import uuid
from blendrelay_blender.asset_rules import AssetPolicy, AssetError
...
class AssetService:
    ...
    def asset_session(self, instance_id=None):
        ...
        if not session or session.get('asset_policy') is None:
            raise AssetError('Connect an updated BlendRelay add-on; select instance_id if multiple Blender instances are open')
        return session
    ...
    def asset_start(self, operation, instance_id=None, **arguments):
        ...
        threading.Thread(target=work,name='BlendRelay-assets',daemon=True).start()

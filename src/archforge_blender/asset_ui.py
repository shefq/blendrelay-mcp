...
def policy(scene):
    return AssetPolicy.read({key:getattr(scene,'archforge_asset_'+key,getattr(AssetPolicy(),key)) for key in FIELDS})
    ...
    if result.get('status')=='failed': STATE['status']=result.get('error','Asset operation failed')
        ...
        if isinstance(data,dict) and 'results' in data:
            ...
            if scene and scene.name in bpy.data.scenes and scene_key(scene)==STATE.get('scene_key'):
                clear_thumbnails()
                scene.archforge_asset_results.clear()
                for record in data['results'][:50]:
                    ...
                    row=scene.archforge_asset_results.add();row.name=record['name'];row.record=json.dumps(record)
                scene.archforge_asset_index=0
...
class AF_AssetResult(bpy.types.PropertyGroup):
    record: StringProperty()


class AF_UL_AssetResults(bpy.types.UIList):
    def draw_item(self,context,layout,data,item,icon,active_data,active_propname,index):
        ...
        op=row.operator('archforge.asset_action',text='',icon='IMPORT');op.action='IMPORT';op.asset_id=record['asset_id']


class AF_OT_AssetAction(bpy.types.Operator):
    bl_idname='archforge.asset_action'
    ...
    def execute(self,context):
        ...
        scene=context.scene
        try:
            rows=scene.archforge_asset_results
            record=json.loads(rows[scene.archforge_asset_index].record) if rows and scene.archforge_asset_index<len(rows) else {}
            if self.action=='OPEN':
                folder=Path(record.get('cache_directory',''))
                root=Path(scene.archforge_runtime_dir)/'assets'
            ...
            connection=Connection(scene.archforge_runtime_dir)
            ...
            if self.action=='SEARCH':
                method='assets.search';params.update(query=scene.archforge_asset_query,provider=None if scene.archforge_asset_provider=='all' else scene.archforge_asset_provider,max_results=20)
...
class AF_PT_AssetLibrary(bpy.types.Panel):
    bl_label='Asset Library'
    bl_idname='AF_PT_AssetLibrary'
    ...
    def draw(self,context):
        ...
        box.prop(scene,'archforge_asset_poly_haven',text='Poly Haven')
        box.label(text='Public catalogue · CC0')
        box.prop(scene,'archforge_asset_poly_pizza',text='Poly Pizza')
        row=box.row(align=True)
        row.prop(scene,'archforge_asset_pizza_cc0',text='CC0');row.prop(scene,'archforge_asset_pizza_cc_by',text='CC-BY')
        box.prop(scene,'archforge_asset_max_download_mb',text='Maximum download (MB)')
        layout.prop(scene,'archforge_asset_query',text='Search')
        layout.prop(scene,'archforge_asset_provider',text='Provider')
        row=layout.row(align=True);row.enabled=not STATE['job']
        row.operator('archforge.asset_action',text='Search',icon='VIEWZOOM').action='SEARCH'
        row.operator('archforge.asset_action',text='Refresh local cache',icon='FILE_REFRESH').action='REFRESH'
        layout.template_list('AF_UL_AssetResults','',scene,'archforge_asset_results',scene,'archforge_asset_index',rows=4)
        rows=scene.archforge_asset_results
        if rows and scene.archforge_asset_index<len(rows):
            record=json.loads(rows[scene.archforge_asset_index].record)
            detail=layout.box()
        ...
        row.operator('archforge.asset_action',text='Import selected asset',icon='IMPORT').action='IMPORT'
        layout.operator('archforge.asset_action',text='Open cached asset location',icon='FILE_FOLDER').action='OPEN'
        for line in textwrap.wrap(STATE['status'],55): layout.label(text=line)
        layout.label(text='Powered by Poly Haven')


CLASSES=(AF_AssetResult,AF_UL_AssetResults,AF_OT_AssetAction,AF_PT_AssetLibrary)
...
def register():
    ...
    for key in FIELDS:
        ...
        setattr(bpy.types.Scene,'archforge_asset_'+key,prop)
    bpy.types.Scene.archforge_asset_query=StringProperty(name='Search',maxlen=200)
    bpy.types.Scene.archforge_asset_provider=EnumProperty(items=[('all','Enabled providers','Search cache then permitted providers'),('local','Local Cache','Search downloaded assets'),('poly_haven','Poly Haven','Public CC0 catalogue'),('poly_pizza','Poly Pizza','Permitted public models')],default='all')
    bpy.types.Scene.archforge_asset_results=CollectionProperty(type=AF_AssetResult,options={'SKIP_SAVE'})
    bpy.types.Scene.archforge_asset_index=IntProperty(default=0,min=0,options={'SKIP_SAVE'})
...
def unregister():
    ...
    for key in FIELDS+('query','provider','results','index'):
        if hasattr(bpy.types.Scene,'archforge_asset_'+key): delattr(bpy.types.Scene,'archforge_asset_'+key)

...
class AF_OT_DrawRegion(bpy.types.Operator):
    bl_idname = 'archforge.draw_target_region'
    ...
    def modal(self, context, event):
        ...
        if event.type == 'LEFTMOUSE' and event.value == 'PRESS':
            ...
            self.guide.name = 'ArchForge Target Region'
            self.guide[TAG] = True
...
class AF_OT_SelectRegion(bpy.types.Operator):
    bl_idname = 'archforge.select_target_region'
    bl_label = 'Adjust Box'
...
class AF_OT_AdjustRegionFaces(bpy.types.Operator):
    bl_idname = 'archforge.adjust_region_faces'
...
class AF_GGT_RegionFaces(bpy.types.GizmoGroup):
    bl_idname = 'AF_GGT_region_faces'
    bl_label = 'ArchForge Region Faces'
...
class AF_OT_RemoveRegion(bpy.types.Operator):
    bl_idname = 'archforge.remove_target_region'
...
class AF_OT_SendRegion(bpy.types.Operator):
    bl_idname = 'archforge.send_region_prompt'
...
def draw(layout, context):
    ...
    guide = box(context.scene)
    if guide is None:
        card.operator('archforge.draw_target_region', text='Draw Target Box')
        card.label(text='Drag width/length; wheel adjusts height.')
        card.label(text='Drawing plane: XY at the 3D cursor height')
    else:
        card.operator('archforge.adjust_region_faces', text='Adjust Six Faces')
        card.operator('archforge.remove_target_region', text='Remove Box', icon='TRASH')
    card.label(text='Describe the change, style and materials.')
    card.prop(context.scene, 'archforge_codex_prompt', text='Prompt')
    row = card.row()
    row.enabled = guide is not None
    row.operator('archforge.send_region_prompt')


CLASSES = (AF_OT_DrawRegion, AF_OT_SelectRegion, AF_OT_AdjustRegionFaces,
           AF_GGT_RegionFaces, AF_OT_RemoveRegion, AF_OT_SendRegion)

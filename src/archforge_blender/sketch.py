...
PROPERTY = 'archforge_viewport_sketch'
TARGET_PROPERTY = 'archforge_viewport_sketch_targets'
MAX_STROKES = 100
...
def strokes(scene):
    try:
        value = json.loads(scene.get(PROPERTY, '[]'))
...
def targets(scene):
    try:
        value = json.loads(scene.get(TARGET_PROPERTY, '[]'))
    except (TypeError, json.JSONDecodeError):
        return []
    if not isinstance(value, list):
        return []

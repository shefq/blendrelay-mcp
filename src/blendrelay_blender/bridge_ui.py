...
def migrate_scene_state(scene):
    if not scene:
        return

def sync_workspace_state(scene=None):
    migrate_scene_state(scene)
    scene = scene or bpy.context.scene
    root = STATE['root'] or getattr(scene, 'blendrelay_runtime_dir', default_root())
...
def reference_images(scene):
    ...
    raw = getattr(scene, 'blendrelay_reference_images', '[]')
    try:
    ...
    if not isinstance(paths, list):
        paths = []
    legacy = getattr(scene, 'blendrelay_reference_image', '').strip()
    if legacy:
    ...
    for value in paths:
        ...
        if path.is_file() and path.suffix.lower() in SUPPORTED_REFERENCE_IMAGE_SUFFIXES:
            ...
            if resolved not in seen:
                seen.add(resolved)
                unique.append(resolved)
    scene.blendrelay_reference_images = json.dumps(unique, ensure_ascii=False)
    scene.blendrelay_reference_image = unique[0] if unique else ''
    return unique
...
def capture_selection_views(root, run_id, selected_objects, angles=(35.0, 155.0, 275.0), max_dimension=384):
    ...
    camera_data = bpy.data.cameras.new(f'BlendRelay selection camera {run_id[:8]}')
...
def ensure_connected(context=None):
    if STATE.get('connection'):
        return True
    try:
        root = context.scene.blendrelay_runtime_dir if context and hasattr(context, 'scene') else (STATE['root'] or default_root())
        STATE['root'] = root
...
def start_agent(context, prompt_override=None, use_region=False, verification_pass=False):
    ...
    if agent == 'ANTIGRAVITY':
        exe_str = getattr(context.scene, 'blendrelay_antigravity_path', '').strip() or default_antigravity_path()
        ...
        if (not executable.is_file() or 'antigravity-ide' in str(executable).lower()) and agy_bin.is_file():
            executable = agy_bin
            try:
                context.scene.blendrelay_antigravity_path = str(agy_bin)
            except Exception:
                pass
        ...
        broad_permission = autonomous or getattr(context.scene, 'blendrelay_unattended_agent_permissions', True)
        narrow_permission = autonomous or getattr(context.scene, 'blendrelay_allow_antigravity_mcp', True)
        if narrow_permission:
            changed, settings_path = agent_permissions.ensure_antigravity_mcp_permission()
            if changed:
                print(f'[BlendRelay] Added narrow Antigravity permission: {agent_permissions.BLENDRELAY_MCP_RULE} in {settings_path}', flush=True)
            registered, config_path = agent_permissions.ensure_antigravity_mcp_server()
            if registered:
                print(f'[BlendRelay] Registered the BlendRelay Antigravity MCP server in {config_path}', flush=True)
        elif not broad_permission and not agent_permissions.antigravity_mcp_is_allowed():
            raise RuntimeError(
                'Antigravity headless prompts need MCP approval. Enable “Allow BlendRelay MCP tools” '
    ...
    root = Path(STATE['root'] or context.scene.blendrelay_runtime_dir).resolve()
    if not context.scene.get('blendrelay_workspace_id'):
        context.scene['blendrelay_workspace_id'] = uuid.uuid4().hex
    scene_root = general.workspace_root(root, context.scene)
    conversation_path = scene_root / 'conversations' / 'state.json'
    resume_id = conversation.load(conversation_path).get(agent) if context.scene.blendrelay_continue_conversation else None
    ...
    only_selected = getattr(context.scene, 'blendrelay_only_selected', False) and not use_region
    auto_verify = getattr(context.scene, 'blendrelay_auto_verify', True)
    resource_mode = getattr(context.scene, 'blendrelay_resource_mode', 'FULL_BUILD')
    output_quality = getattr(context.scene, 'blendrelay_output_quality', 'HIGH')
    ...
    if context.selected_objects and not mesh_edit and only_selected:
        try:
            selection_view_paths = capture_selection_views(root, run_id, context.selected_objects, angles=(35.0,) if not workflow.is_creation(workflow.choose(context.scene.blendrelay_task_mode, prompt, True, context.mode)) else (35.0,155.0,275.0))
        except Exception as error:
    ...
    context_text = {
        ...
        'blendrelay_instance_id': STATE['instance'],
        'agent': agent,
    ...
    task_mode = workflow.choose(context.scene.blendrelay_task_mode, prompt, bool(context.selected_objects), context.mode)
    verification = context.scene.blendrelay_verification
    ...
    compact_ctx = {
        ...
        'instance_id': context_text['blendrelay_instance_id'],
    ...
    if agent == 'ANTIGRAVITY':
        ...
        if 'agy' in exe_lower:
            ...
            args = [
                str(executable),
                *(['--dangerously-skip-permissions'] if (broad_permission or getattr(context.scene,'blendrelay_unattended_agent_permissions',True)) else []),
    ...
    print("\n" + "=" * 60, flush=True)
    print(f"[BlendRelay] Starting {agent_name} task (Model: {model or 'CLI default'})", flush=True)
    print(f"[BlendRelay] User prompt: {prompt}", flush=True)
    print(f"[BlendRelay] Prompt file: {prompt_file} ({prompt_file.stat().st_size:,} bytes)", flush=True)
    if only_selected:
        selected_names = [o.name for o in context.selected_objects[:10]]
        print(f"[BlendRelay] 🎯 Restricted Scope: ONLY selected objects ({len(context.selected_objects)}): {', '.join(selected_names)}", flush=True)
    if region_data:
        print(f"[BlendRelay] 📦 Target Region: bounds {region_data.get('dimensions')} centered at {region_data.get('center')}", flush=True)
    if sketch_viewport:
        print(f"[BlendRelay] Viewport sketch attached: {sketch_viewport}", flush=True)
    if selection_view_paths:
        print(f"[BlendRelay] Attached {len(selection_view_paths)} selected-object reference views", flush=True)
    print(f"[BlendRelay] MCP controls: {max_mcp_calls} calls · {max_edit_attempts} edit attempt(s) · {max_job_polls} job poll(s)", flush=True)
    print(f"[BlendRelay] Log file: {log_path}", flush=True)
    print("=" * 60 + "\n", flush=True)
    ...
    try:
        if task_mode in ('BUILD','EDIT') and not verification_pass:
            ...
            print(f"[BlendRelay] Recovery checkpoint saved (version: {before['version_id'][:8]}).",flush=True)
        ...
        print(f"[BlendRelay] Failed to spawn {agent_name}: {err}", flush=True)
        raise

    reader_thread = threading.Thread(
        target=run_agent_reader,
        args=(process, log_path, agent_name, final_path, conversation_path if context.scene.blendrelay_continue_conversation else None, agent),
        daemon=True,
    )
    reader_thread.start()
...
def check_agent():
    ...
    if verify_after:
        try:
            ...
            quality=getattr(bpy.context.scene,'blendrelay_output_quality','HIGH')
            ...
            verify_prompt=(f'FINAL VISUAL AUDIT AND REPAIR. Original user goal: {original_request}\n'
                ...
                'floating objects, intersections and camera framing. Use BlendRelay to make every useful correction. '
                'Capture a final viewport image after corrections. If it is already strong, preserve it and report that.')
            print('[BlendRelay] Starting automatic visual audit and repair pass.',flush=True)
    ...
    if failure is None:
        ...
        print(f"\n[BlendRelay] {agent_name} task finished successfully (code 0).", flush=True)
        try:
            root = STATE.get('root') or default_root()
            user_prompt = getattr(bpy.context.scene, 'blendrelay_codex_prompt', '').strip() or 'AI edit'
            cp_entry = general.checkpoint(root, f"AI: {user_prompt[:80]}")
            print(f"[BlendRelay] 💾 Checkpoint saved for completed task (version: {cp_entry['version_id'][:8]}).", flush=True)
        except Exception as cp_err:
            print(f"[BlendRelay] Note: could not save post-generation checkpoint: {cp_err}", flush=True)
    else:
        log_name = Path(log_path).name if log_path else 'log'
        STATE['status'] = f'{failure} Open log: {log_name}'
        print(f"\n[BlendRelay] ❌ {failure} Log: {log_path}", flush=True)
    print("=" * 60 + "\n", flush=True)
    refresh_versions()
    redraw()
...
def received(response):
    if 'error' in response:
        print(f"[BlendRelay Bridge] ❌ Error from runtime: {response['error']['message']}", flush=True)
    ...
    if job:
        ...
        print(f"\n[BlendRelay Bridge] ⚡ Executing job '{action}' (op_id: {op_id})...", flush=True)
        ...
        try:
            ...
            if error:
                ...
                print(f"[BlendRelay Bridge] ❌ Job '{action}' failed: {error}", flush=True)
            else:
                ...
                print(f"[BlendRelay Bridge] ✔ Job '{action}' completed: {summary}", flush=True)
        except Exception as e:
            result = None
            error = str(e)
            if STATE.get('active_agent_run_id'):
            ...
            print(f"[BlendRelay Bridge] ❌ Exception in job '{action}': {e}", flush=True)
...
class BR_OT_Connect(bpy.types.Operator):
    bl_idname='blendrelay.refresh';bl_label='Connect / Refresh'
    def execute(self,context):
        if STATE['connection']:STATE['connection'].close()
        STATE['root']=context.scene.blendrelay_runtime_dir
        STATE['connection']=Connection(STATE['root']);STATE['last_poll']=0
        STATE['status']='Connecting…';refresh_versions()
        return {'FINISHED'}


class BR_OT_Disconnect(bpy.types.Operator):
    bl_idname='blendrelay.disconnect';bl_label='Disconnect'
    def execute(self,context):
        if STATE['connection']:STATE['connection'].close()
        STATE['connection']=None;STATE['status']='Disconnected';return {'FINISHED'}


class BR_OT_Checkpoint(bpy.types.Operator):
    bl_idname='blendrelay.checkpoint';bl_label='Save version'
    def execute(self,context):
        try:general.checkpoint(STATE['root'] or context.scene.blendrelay_runtime_dir,'Manual checkpoint');refresh_versions()
        except Exception as e:self.report({'ERROR'},str(e));return {'CANCELLED'}
        return {'FINISHED'}


class BR_OT_VerifyAndFix(bpy.types.Operator):
    bl_idname = 'blendrelay.verify_and_fix'
    ...
    def execute(self, context):
        try:
            ...
            user_prompt = getattr(context.scene, 'blendrelay_codex_prompt', '').strip()
            if user_prompt:
                verify_prompt = (
...
class BR_OT_SendToCodex(bpy.types.Operator):
    bl_idname = 'blendrelay.send_to_codex'
...
class BR_OT_CancelAgent(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if proc:
            ...
            print("\n[BlendRelay] 🛑 Task cancelled by user.\n" + "=" * 60 + "\n", flush=True)
            redraw()
        return {'FINISHED'}


class BR_OT_RefreshModels(bpy.types.Operator):
    bl_idname = 'blendrelay.refresh_models'
...
class BR_OT_AttachReferenceImage(bpy.types.Operator):
    bl_idname = 'blendrelay.attach_reference_image'
    bl_label = 'Attach Reference Image'
    bl_description = 'Attach an image file to guide the BlendRelay AI prompt'
...
class BR_OT_BrowseReferenceImage(bpy.types.Operator):
    bl_idname = 'blendrelay.browse_reference_image'
...
class BR_OT_ClearReferenceImage(bpy.types.Operator):
    bl_idname = 'blendrelay.clear_reference_image'
...
class BR_OT_RemoveReferenceImage(bpy.types.Operator):
    bl_idname = 'blendrelay.remove_reference_image'
...
if hasattr(bpy.types, 'FileHandler'):
    class BR_FH_ImageDrop(bpy.types.FileHandler):
        bl_idname = 'BR_FH_ImageDrop'
        bl_label = 'BlendRelay AI Reference Images'
        bl_import_operator = 'blendrelay.attach_reference_image'
    ...
    BR_FH_ImageDrop = None



class BR_OT_DrawSketch(bpy.types.Operator):
    bl_idname='blendrelay.draw_viewport_sketch';bl_label='Draw viewport sketch'
...
class BR_OT_ClearSketch(bpy.types.Operator):
    bl_idname='blendrelay.clear_viewport_sketch';bl_label='Clear viewport sketch'
    def execute(self,context):
...
class BR_OT_NewSketch(bpy.types.Operator):
    bl_idname='blendrelay.new_viewport_sketch';bl_label='Start a new sketch'
    def execute(self,context):
        sketch.clear(context.scene)
        return bpy.ops.blendrelay.draw_viewport_sketch('INVOKE_DEFAULT')


class BR_OT_RestoreVersion(bpy.types.Operator):
    bl_idname='blendrelay.restore_version';bl_label='Restore version'
    version_id:StringProperty()
    def invoke(self,context,event):return context.window_manager.invoke_confirm(self,event)
    def execute(self,context):
        try:general.restore(STATE['root'] or context.scene.blendrelay_runtime_dir,self.version_id);refresh_versions()
        except Exception as e:self.report({'ERROR'},str(e));return {'CANCELLED'}
        return {'FINISHED'}


class BR_OT_AppendPromptTag(bpy.types.Operator):
    bl_idname = 'blendrelay.append_prompt_tag'
    bl_label = 'Append tag to prompt'
    tag: StringProperty(name='Tag', default='')

    def execute(self, context):
        current = context.scene.blendrelay_codex_prompt.strip()
        if not current:
            context.scene.blendrelay_codex_prompt = self.tag
        elif self.tag.lower() not in current.lower():
            context.scene.blendrelay_codex_prompt = f"{current}, {self.tag}"
        return {'FINISHED'}


class BR_OT_NewConversation(bpy.types.Operator):
    bl_idname = 'blendrelay.new_conversation'
    ...
    def execute(self, context):
        ...
        if process and process.poll() is None:
            ...
            return {'CANCELLED'}
        workspace = context.scene.get('blendrelay_workspace_id')
        if workspace:
            root = Path(STATE['root'] or context.scene.blendrelay_runtime_dir)
            conversation.save(general.workspace_root(root, context.scene) / 'conversations' / 'state.json', context.scene.blendrelay_agent_backend, None)
        self.report({'INFO'}, 'The next prompt will start a new conversation')
        return {'FINISHED'}


class BR_OT_ClearPrompt(bpy.types.Operator):
    bl_idname = 'blendrelay.clear_prompt'
...
class BR_OT_ClearStoredData(bpy.types.Operator):
    bl_idname = 'blendrelay.clear_stored_data'
    ...
    def execute(self, context):
        root = STATE['root'] or context.scene.blendrelay_runtime_dir or default_root()
...
class BR_OT_ToggleViewportHUD(bpy.types.Operator):
    bl_idname = 'blendrelay.toggle_viewport_hud'
    bl_label = 'Toggle Viewport HUD'
    bl_description = 'Show or hide the floating AI Viewport HUD bar'

    def execute(self, context):
        context.scene.blendrelay_show_viewport_hud = not context.scene.blendrelay_show_viewport_hud
        if context.scene.blendrelay_show_viewport_hud:
            viewport_hud.ensure_hud_modal()
        else:
...
class BR_OT_ResetHUDTransform(bpy.types.Operator):
    bl_idname = 'blendrelay.reset_hud_transform'
...
class BR_PT_Main(bpy.types.Panel):
    bl_label = 'BlendRelay AI'
    ...
    def draw(self, context):
        ...
        header_row.operator(
            ...
            text='HUD',
            icon='WINDOW',
            depress=getattr(scene, 'blendrelay_show_viewport_hud', False),
        )
        try:
        ...
        # ── TAB: HISTORY (Checkpoints) ──────────────────────────────────────
            ...
            if not STATE['versions']:
                ...
                for entry in reversed(STATE['versions'][:15]):
                    ...
                    op = row_v.operator('blendrelay.restore_version', text=entry['label'][:38], icon='LOOP_BACK')
...
CLASSES = (
    *target_region.CLASSES,
    ...
    BR_OT_ClearSketch,
    BR_OT_NewSketch,
    BR_OT_RestoreVersion,
    BR_OT_AppendPromptTag,
    BR_OT_ClearPrompt,
    BR_OT_ClearStoredData,
    BR_OT_ToggleViewportHUD,
    BR_OT_ResetHUDTransform,
    BR_OT_RefreshModels,
    BR_OT_AttachReferenceImage,
    BR_OT_BrowseReferenceImage,
    BR_OT_ClearReferenceImage,
    BR_OT_RemoveReferenceImage,
    *( [BR_FH_ImageDrop] if BR_FH_ImageDrop else [] ),
...
def register():
    ...
    bpy.types.Scene.blendrelay_codex_prompt = StringProperty(name='Prompt', default='')
    bpy.types.Scene.blendrelay_auto_verify = BoolProperty(
    ...
    bpy.types.Scene.blendrelay_resource_mode = EnumProperty(
        name='MCP workload',
        description='Choose a tested MCP call budget for the kind of Blender work',
        items=workflow.RESOURCE_MODE_ITEMS,
        default='FULL_BUILD',
    )
    bpy.types.Scene.blendrelay_output_quality = EnumProperty(
    ...
    bpy.types.Scene.blendrelay_permission_mode = EnumProperty(
        name='Generation permissions',description='Autonomous authorizes BlendRelay editing and asset work for this scene',
        items=workflow.PERMISSION_ITEMS,default='AUTONOMOUS')
    bpy.types.Scene.blendrelay_max_mcp_calls = IntProperty(
    ...
    bpy.types.Scene.blendrelay_max_edit_attempts = IntProperty(
    ...
    bpy.types.Scene.blendrelay_max_job_polls = IntProperty(
    ...
    bpy.types.Scene.blendrelay_allow_python_execution = BoolProperty(
        name='Allow arbitrary Python execution',
        description='Permit the destructive execute_blender_python MCP tool for this scene',
        default=True,
    )
    bpy.types.Scene.blendrelay_allow_antigravity_mcp = BoolProperty(
        name='Allow BlendRelay MCP tools',
        description='On the next prompt, add only mcp(blendrelay/*) to Antigravity permissions for headless BlendRelay access',
        default=True,
    )
    bpy.types.Scene.blendrelay_unattended_agent_permissions = BoolProperty(
    ...
    bpy.types.Scene.blendrelay_only_selected = BoolProperty(
        name='Only selected objects',
        description='Restrict edits to selected objects, or selected mesh elements in Edit Mode',
        default=False,
    )
    bpy.types.Scene.blendrelay_ui_tab = EnumProperty(
        name='Panel Tab',
        description='Switch between BlendRelay interface views',
        items=[
    ...
    bpy.types.Scene.blendrelay_agent_backend = EnumProperty(
        name='AI Agent',
    ...
    bpy.types.Scene.blendrelay_reference_images = StringProperty(
        name='Reference Images',
        description='JSON list of image paths attached to the next AI prompt',
        default='[]',
    )

    bpy.types.Scene.blendrelay_show_viewport_hud = BoolProperty(
    ...
    bpy.types.Scene.blendrelay_hud_x_offset = FloatProperty(
        name='HUD X Offset',
        description='Horizontal position offset in the viewport',
        default=0.0,
    )
    bpy.types.Scene.blendrelay_hud_y_offset = FloatProperty(
        name='HUD Y Offset',
...
def unregister():
    ...
    props_to_del = [
        'blendrelay_task_mode',
        'blendrelay_continue_conversation',
        'blendrelay_verification',
        'blendrelay_runtime_dir',
        'blendrelay_codex_prompt',
        'blendrelay_include_selection',
        'blendrelay_only_selected',
        'blendrelay_auto_verify',
        'blendrelay_resource_mode',
        'blendrelay_output_quality',
        'blendrelay_permission_mode',
        'blendrelay_max_mcp_calls',
        'blendrelay_max_edit_attempts',
        'blendrelay_max_job_polls',
        'blendrelay_allow_python_execution',
        'blendrelay_allow_antigravity_mcp',
        ...
        'blendrelay_ui_tab',
        'blendrelay_agent_backend',
        'blendrelay_antigravity_model',
        'blendrelay_antigravity_model_custom',
        'blendrelay_antigravity_path',
...
AF_OT_Connect = BR_OT_Connect
AF_OT_Disconnect = BR_OT_Disconnect
AF_OT_Checkpoint = BR_OT_Checkpoint
AF_OT_VerifyAndFix = BR_OT_VerifyAndFix
AF_OT_SendToAgent = BR_OT_SendToAgent
AF_OT_SendToCodex = BR_OT_SendToCodex
AF_OT_CancelAgent = BR_OT_CancelAgent
AF_OT_RefreshModels = BR_OT_RefreshModels
AF_OT_AttachReferenceImage = BR_OT_AttachReferenceImage
AF_OT_BrowseReferenceImage = BR_OT_BrowseReferenceImage
AF_OT_ClearReferenceImage = BR_OT_ClearReferenceImage
AF_OT_RemoveReferenceImage = BR_OT_RemoveReferenceImage
AF_FH_ImageDrop = BR_FH_ImageDrop
AF_OT_DrawSketch = BR_OT_DrawSketch
AF_OT_ClearSketch = BR_OT_ClearSketch
AF_OT_NewSketch = BR_OT_NewSketch
AF_OT_RestoreVersion = BR_OT_RestoreVersion
AF_OT_AppendPromptTag = BR_OT_AppendPromptTag
AF_OT_NewConversation = BR_OT_NewConversation
AF_OT_ClearPrompt = BR_OT_ClearPrompt
AF_OT_ClearStoredData = BR_OT_ClearStoredData
AF_OT_ToggleViewportHUD = BR_OT_ToggleViewportHUD
AF_OT_ResetHUDTransform = BR_OT_ResetHUDTransform
AF_PT_Main = BR_PT_Main

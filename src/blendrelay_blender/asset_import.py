...
def ensure_gltf_importer():
    ...
    BlendRelay keeps its per-user repair in Blender's add-on modules directory so
...
def import_cached(root, asset_id, asset_scene, collection_name=None, location=None, scale=1.0):
    ...
    try:
        ...
        def child(parent, label):
            found=next((c for c in parent.children if c.name==label and c.get('blendrelay_asset_container')),None)
            if found: return found
            value=bpy.data.collections.new(label);value['blendrelay_asset_container']=True;parent.children.link(value)
            return value
        parent=child(child(scene.collection,'BlendRelay Assets'),'Poly Haven' if provider=='poly_haven' else 'Poly Pizza')
        ...
        for block in [collection]+imported+materials+worlds:
            block['blendrelay_asset_metadata']=json.dumps(metadata,ensure_ascii=False)
            block['blendrelay_asset_id']=asset_id
        ...
        for empty in set(bpy.data.collections)-before_collections:
            if empty!=collection and not empty.get('blendrelay_asset_container') and not empty.objects and not empty.children:

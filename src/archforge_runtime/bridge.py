...
class Bridge:
    ...
    def rpc_budget_claim(self,kind='other',instance_id=None,operation_id=None):
        ...
        if instance_id is None:
            if len(sessions)==1:
                ...
                if len(active)==1:instance_id=active[0]
            if instance_id is None:
        ...
        if not session or (kind!='poll' and time.monotonic()-session['seen']>=LIVE_SESSION_SECONDS):
            raise DomainError('BLENDER_OFFLINE','Reconnect the ArchForge panel')
        ...
        session['mcp_calls']+=1
        if kind=='poll':session['job_polls']+=1
    ...
    def rpc_budget_commit(self, kind='edit', instance_id=None):
        if kind not in ('edit','asset'):raise DomainError('INVALID_BUDGET_KIND','Only accepted edits are committed')
        session=self.blender_sessions.get(instance_id)
        if not session or not session.get('agent_run_id'):return {'enforced':False}
    ...
    def rpc_blender_submit(self, operation_id=None, action=None, arguments=None, instance_id=None):
        ...
        if instance_id is None:
            ...
            if len(candidates)!=1:raise DomainError('SELECT_INSTANCE','Connect Blender; if several instances are open, specify instance_id')
        ...
        busy=any(job.get('instance_id')==instance_id and job.get('status')=='running' and time.time()-job.get('created_at',0)<BUSY_SESSION_SECONDS
                 for job in (json.loads(path.read_text()) for path in self.bridge_dir.glob('*.json')))
        if not live and not busy:raise DomainError('BLENDER_OFFLINE','Reconnect the ArchForge panel')
        session=self.blender_sessions[instance_id]
        if action in ('execute','execute_code') and not session.get('allow_python_execution'):
            raise DomainError('PYTHON_EXECUTION_DISABLED','Enable “Allow arbitrary Python execution” in ArchForge Settings for this scene')

"""Local authenticated runtime for ArchForge's Blender bridge."""
import argparse
import json
import os
import secrets
...
from .errors import ArchForgeError
...
def serve(root, source_roots=()):
        ...
        def handle(self):
            ...
            while True:
                ...
                try:
                    ...
                    if not isinstance(request, dict): raise ArchForgeError("INVALID_MESSAGE", "Expected an envelope")
                    if not secrets.compare_digest(str(request.get("token", "")), token):
                        raise ArchForgeError("UNAUTHORIZED", "Invalid session token")
                    if request.get("protocol_version") != 1:
                        raise ArchForgeError("PROTOCOL_VERSION", "Expected bridge protocol 1")
                    ...
                    print(f"ArchForge request error: {type(error).__name__}: {error}", file=sys.stderr)
    ...
    connection = {"product": "ArchForge MCP", "protocol_version": 1,
...
def main():
    parser = argparse.ArgumentParser(description="ArchForge MCP local Blender runtime")

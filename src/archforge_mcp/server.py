...
tool('archforge_blender_command','blender.submit','Operate on the open Blender scene. Actions: inspect (offset, limit, object_name), execute (code, label), versions (no arguments), checkpoint (label), restore (version_id), screenshot (no arguments). execute runs unrestricted Blender Python on its main thread with bpy available; set result to return JSON and print for output. Every execution saves before/after full-scene versions. Read scene first, preserve unrelated content, and use a stable operation_id for retries. No architectural geometry restrictions. Python has the same filesystem/network access as Blender; only run code authorized by the user. Returns a job; poll archforge_blender_job until complete/failed. Failed code may leave partial edits; restore its before version if needed.',

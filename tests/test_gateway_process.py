    ...
    def test_stdio_gateway_autostarts_authenticated_runtime(self):
        with tempfile.TemporaryDirectory() as tmp:
            messages = [
                ...
                {'jsonrpc': '2.0', 'id': 3, 'method': 'tools/call', 'params': {'name': 'blendrelay_get_capabilities', 'arguments': {}}},
            ]
            env = dict(os.environ, PYTHONPATH=str(Path(__file__).resolve().parents[1] / 'src'))
            try:
                result = subprocess.run(
                    [sys.executable, '-m', 'blendrelay_mcp.server', '--data-dir', tmp],
                ...
                self.assertIn('blendrelay_get_capabilities', tool_names)

                # Canonical tool call check
                self.assertFalse(responses[2]['result']['isError'])
                capabilities = responses[2]['result']['structuredContent']
                self.assertEqual(capabilities['mcp_protocol'], '2026-07-28')

                from blendrelay_runtime.protocol import receive, frame
                import socket

...
import bpy
from archforge_blender import general,register,unregister
...
with nullcontext(test_root/('general-'+uuid.uuid4().hex)) as root:
    ...
    assert bpy.app.timers.is_registered(__import__('archforge_blender.bridge_ui',fromlist=['timer']).timer)
    unregister()
    print('ARCHFORGE_GENERAL_OK: arbitrary mesh, preservation, failure checkpoint, full-scene restore, persistent timer')

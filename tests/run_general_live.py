...
from blendrelay_runtime.protocol import Client

project = Path(__file__).resolve().parents[1]
root = project / '.test-output' / ('general-live-' + uuid.uuid4().hex)
root.mkdir(parents=True)
env = dict(os.environ, PYTHONPATH=str(project / 'src'), BLENDRELAY_TEST_ROOT=str(root))
...
with (root / 'runtime.log').open('w') as log, (root / 'blender.log').open('w') as blog:
    try:
        runtime = subprocess.Popen([sys.executable, '-m', 'blendrelay_runtime.cli', '--data-dir', str(root), 'serve'], env=env, stdout=log, stderr=log, startupinfo=startup)
        ...
        while True:
            try:
                if client.call('blender.sessions'):
                    break
            except (OSError, Exception):
                pass
            if time.monotonic() > until:
                raise RuntimeError('Blender connection timed out')
            time.sleep(.25)
        ...
        def run(action, arguments=None):
            ...
            while job['status'] in ('queued', 'running'):
                if time.monotonic() > deadline:
                    raise RuntimeError('Job timeout: ' + action)
                time.sleep(.15)
                job = client.call('blender.job', {'operation_id': op})
            assert job['status'] == 'complete', job
            return job['result']

        scene = run('inspect')
        assert scene['total'] == 3
        edit = run('execute', dict(code="br.monkey('Live sculpture', location=(0,0,4))\nresult={'name':bpy.context.object.name}", label='Live sculpture', save_checkpoint=True))
